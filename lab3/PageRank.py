...
def compute_pageranks(g,d):
    ...
    P  = dict.fromkeys(g.keys(),1/n)
    dist =  float('inf')
    list_vertices = list(g)
    iterations = 0
    while dist > 0.01 :
        Pnew  = dict.fromkeys(g.keys(),(1-d)/n) 
        for key in g.keys():
            L  = g[key]
            for j in L :
                Pnew[j]+= d*(P[key]/len(L))
        #compute the distance
        pv = np.array([])
        pnewv  = np.array([])
        for w in P.keys() :
            pv = np.append(pv,P[w])
            pnewv = np.append(pnewv,Pnew[w])
        dist = norm(pnewv-pv)  
        print(dist)   
        P = Pnew 
        iterations+=1
...
def rank_simple_graph():
    damping_factor = 0.85  # to get the slide example. Change
    time1 = time.time()
